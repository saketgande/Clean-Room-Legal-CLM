...
# Periodic maintenance run by celery beat (service defined in
# docker-compose.prod.yml). Each task is import-safe and a no-op when its
# tables are empty, so enabling beat never destabilises a fresh deployment.
