...
class ApprovalsService:
    ...
    async def _activate_step(
        ...
        db = self.db
        recipients = self._step_recipients(approval=approval)
        if not recipients:
            return None
    ...
    async def submit_contract_for_approval(
        ...
        *,
        ...
        return await self.submit_subject_for_approval(
            user=user,
            subject=subject,
            approver_user_id=approver_user_id,
            approver_role=approver_role,
    ...
    async def reassign_rung(
        ...
        *,
        ...
        email_sent = await self._activate_step(approval=approval, subject=subject, requester=requester)
        write_audit_log(
    ...
    def get_review_context_for_token(self, *, token: str) -> dict:
...
async def redeem_token_decision(
    ...
    *,
    ...
    return await ApprovalsService(db).redeem_token_decision(
        token=token, decision=decision, comment=comment, request_id=request_id
    )


def get_review_context_for_token(db: Session, *, token: str) -> dict:
    return ApprovalsService(db).get_review_context_for_token(token=token)
