...
def _can_see(workflow: Workflow, user_id: str) -> bool:
...
def _load_owned_workflow(db: Session, workflow_id: str, current_user) -> Workflow:
...
def _snapshot(db: Session, workflow: Workflow, *, actor_user_id: str, note: str | None = None) -> None:
...
def _serialize_version(v: WorkflowVersion) -> dict[str, Any]:
    return {
        "id": v.id,
...
def list_workflows(
    ...
    rows = db.scalars(
        select(Workflow).where(
            Workflow.org_id == current_user.org_id, Workflow.deleted_at.is_(None)
        )
    ).all()
    ...
    return [*visible, *builtin_workflows()]
