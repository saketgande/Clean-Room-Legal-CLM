...
A **Workflow** is a reusable, versioned definition: selection *criteria* + an ordered
list of typed *steps* (stored inline as JSON — the builder just reorders the
list, no separate step table). A **WorkflowRun** is one ticket walking one flow; a
**WorkflowStepRun** is the per-step audit row that powers both the ticket stepper
and the timeline.
"""
...
class WorkflowRun(TableNameMixin, IdMixin, OrgScopedMixin, ActorTrackedMixin, TimestampMixin, Base):
    ...
    flow_id = Column(String(36), ForeignKey("workflow.id", ondelete="SET NULL"), nullable=True)
...
class WorkflowStepRun(TableNameMixin, IdMixin, OrgScopedMixin, TimestampMixin, Base):
    ...
    flow_run_id = Column(
        String(36), ForeignKey("workflow_run.id", ondelete="CASCADE"), index=True, nullable=False
    )
