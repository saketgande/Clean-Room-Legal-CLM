...
def granted_resource_ids(user: User, resource_type: str, min_level: str = "read"):
    ...
    Stays a module-level function (no ``db`` needed) — it's imported and
    called directly by contracts/access.py to build a SQL filter clause.
    """
    return (
...
class GrantService:
    """Object-level resource grants: row checks, management gate, CRUD.
    ...
    reading ``self.db``. ``granted_resource_ids`` and the SQL-predicate
    helpers above stay module-level since they don't touch ``db``.
    """
    ...
    def user_has_grant(
        self, *, user: User, resource_type: str, resource_id: str, min_level: str = "read"
    ) -> bool:
    ...
    def _resource_owner_id(self, resource_type: str, resource_id: str) -> str | None:
        db = self.db
    ...
    def can_manage_grants(self, *, user: User, resource_type: str, resource_id: str) -> bool:
        ...
        if is_org_admin(user):
            return True
        if self._resource_owner_id(resource_type, resource_id) == user.id:
            return True
        return self.user_has_grant(
            user=user, resource_type=resource_type, resource_id=resource_id, min_level="share"
        )
    ...
    def serialize_grant(self, grant: ResourceGrant) -> dict:
        now = utcnow()
        ...
        return {
            ...
            "principal_label": self._principal_label(grant.principal_type, grant.principal_id),
    ...
    def list_grants_for_resource(
        self, *, org_id: str, resource_type: str, resource_id: str, include_revoked: bool = False
    ) -> list[dict]:
        ...
        rows = self.db.scalars(q.order_by(ResourceGrant.created_at.desc())).all()
    ...
    def grant_access(self, *, actor: User, payload) -> dict:
        ...
        if not self.can_manage_grants(
            user=actor, resource_type=payload.resource_type, resource_id=payload.resource_id
        ):
            raise HTTPException(status.HTTP_403_FORBIDDEN, "Not allowed to share this resource")
        self._validate_principal(
            org_id=actor.org_id, principal_type=payload.principal_type, principal_id=payload.principal_id
        )
    ...
    def revoke_grant(self, *, actor: User, grant_id: str) -> None:
        ...
        if not self.can_manage_grants(
            user=actor, resource_type=grant.resource_type, resource_id=grant.resource_id
        ):
...
# user_has_grant is imported directly by contracts/access.py (module-level).
...
def user_has_grant(
    ...
    return GrantService(db).user_has_grant(
        user=user, resource_type=resource_type, resource_id=resource_id, min_level=min_level
    )


def can_manage_grants(db: Session, *, user: User, resource_type: str, resource_id: str) -> bool:
    return GrantService(db).can_manage_grants(
        user=user, resource_type=resource_type, resource_id=resource_id
    )


def serialize_grant(db: Session, grant: ResourceGrant) -> dict:
    return GrantService(db).serialize_grant(grant)


def list_grants_for_resource(
    db: Session, *, org_id: str, resource_type: str, resource_id: str, include_revoked: bool = False
) -> list[dict]:
    return GrantService(db).list_grants_for_resource(
        org_id=org_id, resource_type=resource_type, resource_id=resource_id, include_revoked=include_revoked
    )


def grant_access(db: Session, *, actor: User, payload) -> dict:
    return GrantService(db).grant_access(actor=actor, payload=payload)


def revoke_grant(db: Session, *, actor: User, grant_id: str) -> None:
    return GrantService(db).revoke_grant(actor=actor, grant_id=grant_id)
