...
* The synchronous path is preserved behind a config flag for environments
  that explicitly want the old behaviour.
"""
...
def _flush_sync(row: dict[str, Any]) -> None:
    """Write a single row inline. Used only when async batching is disabled."""
    db = SessionLocal()
    try:
...
def enqueue(row: dict[str, Any]) -> None:
    """Submit a row for the batched writer; drop on overflow.
    ...
    try:
        ...
        # Request logs are observability data, so drop on overflow rather than
        # doing synchronous DB work on the request path.
        global _DROPPED
        with _DROPPED_LOCK:
            _DROPPED += 1
        ...
        if should_log:
            logger.warning(
                "request_log queue full; dropping request log row (total=%d)",
                _DROPPED,
            )
        return
