...
class TeamService:
    ...
    def _resolve_chain(self, team_id: str) -> list[IntakeTeam]:
    ...
    def pick_from_pool(self, *, team_id: str) -> PoolPick | None:
        ...
        for i, t in enumerate(chain):
            ...
            return PoolPick(
                ...
                user_name=self._label(pick.user_id) or pick.user_id, overflow=i > 0,
            )
        return None  # all at capacity, no overflow left → stays unassigned

    # --- CRUD ------------------------------------------------------------------

    def serialize_team(self, t: IntakeTeam) -> dict:
        db = self.db
    ...
    def _check_no_cycle(self, *, org_id: str, team_id: str | None, overflow_id: str | None) -> None:
    ...
    def update_team(self, *, actor: User, team_id: str, payload) -> dict:
        db = self.db
        t = self._get_team(actor.org_id, team_id)
        if payload.overflow_team_id is not None and payload.overflow_team_id:
            self._get_team(actor.org_id, payload.overflow_team_id)
            self._check_no_cycle(org_id=actor.org_id, team_id=t.id, overflow_id=payload.overflow_team_id)
...
def serialize_team(db: Session, t: IntakeTeam) -> dict:
    return TeamService(db).serialize_team(t)
...
def create_team(db: Session, *, actor: User, payload) -> dict:
    return TeamService(db).create_team(actor=actor, payload=payload)


def update_team(db: Session, *, actor: User, team_id: str, payload) -> dict:
    return TeamService(db).update_team(actor=actor, team_id=team_id, payload=payload)


def delete_team(db: Session, *, actor: User, team_id: str) -> None:
    return TeamService(db).delete_team(actor=actor, team_id=team_id)
