...
class RoutingService:
    """No-code when->then routing rule evaluation and CRUD.

    Part of the DI migration (see backend/DI_MIGRATION.md). Constructed with
    a ``db`` session; ``teams`` defaults to a ``TeamService`` built from the
    same session (composition, since routing needs the pool balancer).
    """

    def __init__(self, db: Session, *, teams: TeamService | None = None):
        self.db = db
        self.teams = teams or TeamService(db)

    def apply_routing(self, request: IntakeRequest) -> None:
        db = self.db
        ...
        for r in rules:
            ...
            if r.set_assignee_user_id and r.set_assignee_user_id != w.assignee:
                ...
                actions.append(f"assignee → {self.teams._label(r.set_assignee_user_id)}")
            elif r.set_team_id:
                pick = self.teams.pick_from_pool(team_id=r.set_team_id)
            ...
            if r.escalate_to_user_id:
                ...
                if changed:
                    w.escalated = True
                    actions.append(f"escalate → {self.teams._label(r.escalate_to_user_id)}")
            if r.require_approval_from_user_id and r.require_approval_from_user_id != w.gate:
                w.gate = r.require_approval_from_user_id
                actions.append(f"approval gate → {self.teams._label(r.require_approval_from_user_id)}")
            if actions:
    ...
    def serialize_rule(self, r: IntakeRoutingRule) -> dict:
        db = self.db
        ...
        return {
            ...
            "set_assignee_name": self.teams._label(r.set_assignee_user_id),
            ...
            "escalate_to_name": self.teams._label(r.escalate_to_user_id),
            "require_approval_from_user_id": r.require_approval_from_user_id,
            "require_approval_from_name": self.teams._label(r.require_approval_from_user_id),
    ...
    def list_rules(self, *, org_id: str) -> list[dict]:
        ...
        return [self.serialize_rule(r) for r in rows]

    def create_rule(self, *, actor: User, payload) -> dict:
        db = self.db
        ...
        db.commit()
        db.refresh(r)
        return self.serialize_rule(r)

    def _get_rule(self, org_id: str, rule_id: str) -> IntakeRoutingRule:
        r = self.db.get(IntakeRoutingRule, rule_id)
        if r is None or r.org_id != org_id:
            raise HTTPException(404, "Rule not found")
        return r

    def update_rule(self, *, actor: User, rule_id: str, payload) -> dict:
        db = self.db
        r = self._get_rule(actor.org_id, rule_id)
    ...
    def delete_rule(self, *, actor: User, rule_id: str) -> None:
        db = self.db
        r = self._get_rule(actor.org_id, rule_id)
...
def apply_routing(db: Session, request: IntakeRequest) -> None:
    return RoutingService(db).apply_routing(request)


def serialize_rule(db: Session, r: IntakeRoutingRule) -> dict:
    return RoutingService(db).serialize_rule(r)


def list_rules(db: Session, *, org_id: str) -> list[dict]:
    return RoutingService(db).list_rules(org_id=org_id)


def create_rule(db: Session, *, actor: User, payload) -> dict:
    return RoutingService(db).create_rule(actor=actor, payload=payload)


def update_rule(db: Session, *, actor: User, rule_id: str, payload) -> dict:
    return RoutingService(db).update_rule(actor=actor, rule_id=rule_id, payload=payload)


def delete_rule(db: Session, *, actor: User, rule_id: str) -> None:
    return RoutingService(db).delete_rule(actor=actor, rule_id=rule_id)
