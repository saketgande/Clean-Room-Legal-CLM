...
class IngestService:
    ...
    ``create_request``). Pure helpers (secret/HMAC checks, rate limiting, the
    Graph token fetch) stay module-level, unchanged, above.
    """

    def __init__(self, db: Session, *, intake: IntakeService | None = None):
        self.db = db
        self.intake = intake or IntakeService(db)

    # ---- shared ingest core --------------------------------------------------------

    def _resolve_requester(self, org_id: str, email: str | None) -> User:
    ...
    def ingest_message(
        ...
        requester = self._resolve_requester("org", from_email)
    ...
    def _get_watermark(self) -> str | None:
        row = self.db.query(AdminSetting).filter(AdminSetting.key == WATERMARK_KEY).first()
    ...
    def poll_mailbox(self) -> dict:
        ...
        token = _graph_token()
        anchor_user = self._resolve_requester("org", None)  # org for the watermark row
...
# _resolve_requester and ingest_message are imported directly by
# app.intake.gmail_sync (also converted this pass, but composes an
# IngestService directly rather than going through these wrappers — see
# gmail_sync.py). Kept for any other caller. Tracked in backend/DI_MIGRATION.md.

def _resolve_requester(db: Session, org_id: str, email: str | None) -> User:
    return IngestService(db)._resolve_requester(org_id, email)
...
def ingest_message(
    ...
    return IngestService(db).ingest_message(
        source=source, from_email=from_email, subject=subject,
