...
from app.contracts.access import ContractAccessService, accessible_contract_filter
...
class ContractService:
    """Contract read/update operations.

    Part of the DI migration (see backend/DI_MIGRATION.md). Constructed with a
    ``db`` session; every method that used to take ``db`` as its first
    positional argument now reads ``self.db`` instead. ``access`` defaults to
    a ``ContractAccessService`` built from the same session but can be
    overridden (e.g. with a fake) in tests.
    """

    def __init__(self, db: Session, *, access: ContractAccessService | None = None):
        self.db = db
        self.access = access or ContractAccessService(db)

    def get_contract_for_user(self, *, contract_id: str, user: User) -> Contract:
        db = self.db
        ...
        if (
            ...
            or not self.access.user_can_access_contract(contract=contract, user=user)
        ):
    ...
    def list_contract_stage_history(self, *, contract: Contract) -> list[ContractStageHistory]:
        return self.db.scalars(
    ...
    def list_contract_activity(self, *, contract: Contract, limit: int = 100) -> list[ResourceTimelineEvent]:
        return self.db.scalars(
    ...
    def compute_review_status(self, *, contract: Contract) -> dict:
        ...
        open_comments = self._open_comment_count(contract_id=cid)
    ...
    def _open_comment_count(self, *, contract_id: str) -> int:
    ...
    def compute_version_diff(
        self, *, contract: Contract, base_version_id: str, target_version_id: str
    ) -> dict:
    ...
    def list_contract_parties(self, *, contract: Contract) -> list[ContractParty]:
        return list(
            self.db.scalars(
    ...
    def delete_contract_party(self, *, contract: Contract, party_id: str) -> None:
        db = self.db
    ...
    def list_signer_options(self, *, contract: Contract) -> list[dict]:
        ...
        for party in self.list_contract_parties(contract=contract):
...
# they're migrated to Depends(get_contract_service) in their own passes.
...
def list_contracts_for_user(
    db: Session, *, user: User, limit: int = 100, offset: int = 0
) -> list[Contract]:
    return ContractService(db).list_contracts_for_user(user=user, limit=limit, offset=offset)


def update_contract_metadata(
    db: Session, *, contract: Contract, user: User, updates: dict, request_id: str | None = None
) -> Contract:
    return ContractService(db).update_contract_metadata(
        contract=contract, user=user, updates=updates, request_id=request_id
    )


def list_contract_stage_history(db: Session, *, contract: Contract) -> list[ContractStageHistory]:
    return ContractService(db).list_contract_stage_history(contract=contract)


def list_contract_activity(db: Session, *, contract: Contract, limit: int = 100) -> list[ResourceTimelineEvent]:
    return ContractService(db).list_contract_activity(contract=contract, limit=limit)


def compute_review_status(db: Session, *, contract: Contract) -> dict:
    return ContractService(db).compute_review_status(contract=contract)


def compute_version_diff(
    db: Session, *, contract: Contract, base_version_id: str, target_version_id: str
) -> dict:
    return ContractService(db).compute_version_diff(
        contract=contract, base_version_id=base_version_id, target_version_id=target_version_id
    )


def list_contract_parties(db: Session, *, contract: Contract) -> list[ContractParty]:
    return ContractService(db).list_contract_parties(contract=contract)
...
def add_contract_party(
    ...
    *,
    ...
    return ContractService(db).add_contract_party(
        contract=contract, user=user, name=name, contact_email=contact_email, party_type=party_type
    )


def delete_contract_party(db: Session, *, contract: Contract, party_id: str) -> None:
    return ContractService(db).delete_contract_party(contract=contract, party_id=party_id)


def list_signer_options(db: Session, *, contract: Contract) -> list[dict]:
    return ContractService(db).list_signer_options(contract=contract)
