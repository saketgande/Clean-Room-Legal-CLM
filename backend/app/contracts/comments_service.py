...
class ContractCommentService:
    """Internal and counterparty-visible comments on a contract.
    ...
    def list_comments(self, *, contract: Contract, include_resolved: bool = True) -> list[dict]:
        ...
        names = self._name_map(contract.org_id)
    ...
    def create_comment(
        ...
        *,
        ...
        db.commit()
        db.refresh(comment)
        return self._serialize(comment, names=self._name_map(contract.org_id))

    # --- Counterparty (external share) side ----------------------------------
    def list_shared_comments(self, *, contract: Contract) -> list[dict]:
    ...
    def _get_owned(self, *, contract: Contract, comment_id: str) -> ContractComment:
        comment = self.db.get(ContractComment, comment_id)
        if (
            comment is None
    ...
    def set_resolved(self, *, contract: Contract, user: User, comment_id: str, resolved: bool) -> dict:
        comment = self._get_owned(contract=contract, comment_id=comment_id)
    ...
    def delete_comment(self, *, contract: Contract, user: User, comment_id: str) -> dict:
        comment = self._get_owned(contract=contract, comment_id=comment_id)
...
def list_shared_comments(db: Session, *, contract: Contract) -> list[dict]:
    return ContractCommentService(db).list_shared_comments(contract=contract)


def add_counterparty_comment(
    db: Session, *, contract: Contract, author_name: str | None, body: str, request_id: str | None = None
) -> dict:
    return ContractCommentService(db).add_counterparty_comment(
        contract=contract, author_name=author_name, body=body, request_id=request_id
    )
