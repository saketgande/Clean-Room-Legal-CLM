...
class AuthService:
    """Registration, login, tokens, invitations, API keys, user approval.

    Part of the DI migration (see backend/DI_MIGRATION.md). Constructed with
    a ``db`` session (request-scoped); every function that used to take
    ``db`` as its first positional argument is now a method reading
    ``self.db``. Pure formatting helpers (``_user_response``,
    ``_public_invitation``, ``_public_api_key``, ``as_user_response``, email
    masking) stay module-level — they touch neither ``db`` nor any client.
    """

    def __init__(self, db: Session):
        self.db = db

    def _role_by_name(self, *, org_id: str, role_name: str) -> Role:
        role = self.db.scalar(select(Role).where(Role.org_id == org_id, Role.name == role_name))
        if role is None:
    ...
    def _find_refresh_token(self, raw_token: str | None) -> RefreshToken:
    ...
    def bootstrap_roles(self, org_id: str, actor_user_id: str | None = None) -> dict[str, Role]:
        db = self.db
    ...
    def create_first_admin(self, payload: SetupAdminRequest, request_id: str | None = None) -> User:
        db = self.db
        ...
        roles = self.bootstrap_roles(org.id)
        ...
        self.bootstrap_roles(org.id, actor_user_id=user.id)
    ...
    def register_user(self, payload: RegisterRequest) -> tuple[str, User | None]:
        db = self.db
    ...
    def login_user(self, email: str, password: str, request_id: str | None = None) -> dict:
        db = self.db
    ...
    def refresh_login_tokens(
        ...
        *,
        ...
        db = self.db
        row = self._find_refresh_token(refresh_token)
    ...
    def is_access_token_revoked(
        self, *, jti: str, user_id: str | None, token_issued_at: datetime | None = None
    ) -> bool:
    ...
    def create_user_invitation(
        ...
        *,
        ...
        db = self.db
        self._role_by_name(org_id=actor.org_id, role_name=payload.role_name)
        existing_user = db.scalar(select(User).where(User.email == str(payload.email).lower()))
    ...
    def list_user_invitations(self, *, actor: User) -> list[dict]:
    ...
    def accept_user_invitation(
        ...
        *,
        ...
        role = self._role_by_name(org_id=invitation.org_id, role_name=invitation.role_name)
    ...
    def create_api_key(self, *, payload: ApiKeyCreate, actor: User, request_id: str | None = None) -> dict:
        db = self.db
    ...
    def list_api_keys(self, *, actor: User) -> list[dict]:
        rows = self.db.scalars(
            select(ApiKey)
...
# Every function below is a thin delegate to AuthService, kept so external
# importers (notably backend/app/core/deps.py on the hot request path, and
# backend/app/devtools.py) keep working unchanged while they're migrated to
# Depends(get_auth_service) in their own passes. Tracked in
...
def bootstrap_roles(db: Session, org_id: str, actor_user_id: str | None = None) -> dict[str, Role]:
    return AuthService(db).bootstrap_roles(org_id, actor_user_id=actor_user_id)


def create_first_admin(db: Session, payload: SetupAdminRequest, request_id: str | None = None) -> User:
    return AuthService(db).create_first_admin(payload, request_id=request_id)
...
def login_user(db: Session, email: str, password: str, request_id: str | None = None) -> dict:
    return AuthService(db).login_user(email, password, request_id=request_id)
...
def refresh_login_tokens(
    ...
    *,
    ...
    return AuthService(db).refresh_login_tokens(refresh_token=refresh_token, request_id=request_id)


def is_access_token_revoked(
    db: Session, *, jti: str, user_id: str | None, token_issued_at: datetime | None = None
) -> bool:
    return AuthService(db).is_access_token_revoked(
        jti=jti, user_id=user_id, token_issued_at=token_issued_at
    )
...
def revoke_refresh_token(
    ...
    *,
    ...
    return AuthService(db).revoke_refresh_token(
        user=user,
        refresh_token=refresh_token,
...
def list_org_users(
    ...
    *,
    ...
    return AuthService(db).list_org_users(actor=actor, status_filter=status_filter, limit=limit)
...
def decide_user_approval(
    ...
    *,
    ...
    return AuthService(db).decide_user_approval(
...
def create_user_invitation(
    ...
    *,
    ...
    return AuthService(db).create_user_invitation(payload=payload, actor=actor, request_id=request_id)


def list_user_invitations(db: Session, *, actor: User) -> list[dict]:
    return AuthService(db).list_user_invitations(actor=actor)
...
def revoke_user_invitation(
    ...
    *,
    ...
    return AuthService(db).revoke_user_invitation(
        invitation_id=invitation_id, actor=actor, request_id=request_id
    )
...
def accept_user_invitation(
    ...
    *,
    ...
    return AuthService(db).accept_user_invitation(payload=payload, request_id=request_id)
...
def request_password_reset(
    ...
    *,
    ...
    return AuthService(db).request_password_reset(email=email, request_id=request_id)
...
def confirm_password_reset(
    ...
    *,
    ...
    return AuthService(db).confirm_password_reset(payload=payload, request_id=request_id)


def create_api_key(db: Session, *, payload: ApiKeyCreate, actor: User, request_id: str | None = None) -> dict:
    return AuthService(db).create_api_key(payload=payload, actor=actor, request_id=request_id)


def list_api_keys(db: Session, *, actor: User) -> list[dict]:
    return AuthService(db).list_api_keys(actor=actor)
...
def revoke_api_key(
    ...
    *,
    ...
    return AuthService(db).revoke_api_key(api_key_id=api_key_id, actor=actor, request_id=request_id)


def authenticate_api_key(db: Session, raw_key: str) -> User | None:
    return AuthService(db).authenticate_api_key(raw_key)
...
def switch_active_role(
    ...
    *,
    ...
    return AuthService(db).switch_active_role(
        actor=actor, role_id=role_id, role_name=role_name, request_id=request_id
    )
