    ...
    <>
      {res.semantic.length > 0 && (
        <div className="sec"><div className="seclbl">{svg('<path d="M12 3l2 5 5 2-5 2-2 5-2-5-5-2 5-2z"/>')}Closest in meaning<span className="hintdim"> · semantic — not just keywords</span></div>
          <div className="srclist">{res.semantic.map((s, i) => <SourceRow key={i} contractId={s.contract_id} title={s.contract_title} excerpt={`…${s.text}…`} badge={<>{s.cited && <span className="tag cited">Cited</span>}<span className="tag blue">{Math.round(s.score * 100)}%</span></>} />)}</div>
        </div>
      )}
...
const BRN_CSS = `
.brn{--shadow:0 1px 2px rgba(20,26,40,.05),0 8px 22px rgba(20,26,40,.06);--sans:var(--font-sans);--mono:ui-monospace,SFMono-Regular,Menlo,monospace;height:100%;display:flex;flex-direction:column;min-height:0;color:var(--ink);font:400 13px/1.5 var(--sans)}
.dark .brn{--shadow:0 1px 2px rgba(0,0,0,.4),0 10px 26px rgba(0,0,0,.4)}
...
.brn .tag.cited{background:var(--good-soft);color:var(--good);margin-right:6px;letter-spacing:.02em}
