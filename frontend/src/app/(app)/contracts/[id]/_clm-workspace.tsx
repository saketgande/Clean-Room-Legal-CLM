...
export function ClmWorkspace({ id }: { id: string }) {
  ...
  const { data: contract } = useQuery({ queryKey: ["contract", id], queryFn: () => contractsApi.get(id) });
                ...
                {tab === "assistant" && (
                  <>
                    ...
                    <div className="inputbar">
                      <input value={q} placeholder="Ask the agent to redline or analyse…" onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") send(q); }} />
                      <button className="send" onClick={() => send(q)}>{svg('<path d="M22 2 11 13M22 2l-7 20-4-9-9-4z"/>')}</button>
                    </div>
                  </>
                )}
...
const CLM_CSS = `
.clm{--bg:#f4f6f9;--surface:#fff;--surface-2:#eef1f6;--inset:#f8fafc;--ink:#18213a;--ink-2:#586178;--ink-3:#8a92a6;--border:#e4e8f0;--border-strong:#ccd3e0;--accent:#3b4aa0;--accent-ink:#fff;--accent-soft:#eaecf8;--good:#2f875f;--good-soft:#e4f1ea;--warn:#a9772b;--warn-soft:#f6edd9;--crit:#bb4835;--crit-soft:#f7e4df;--ext:#75589f;--ext-soft:#efe8f7;--ai:#5a49c0;--ai-soft:#ecebf9;--shadow:0 1px 2px rgba(20,26,40,.05),0 10px 26px rgba(20,26,40,.06);--pop:0 12px 30px rgba(20,26,40,.16);--mono:ui-monospace,SFMono-Regular,Menlo,monospace;--sans:system-ui,-apple-system,"Segoe UI",Roboto,sans-serif;--serif:"Iowan Old Style",Georgia,"Times New Roman",serif;height:100%;background:var(--bg);color:var(--ink);font:400 13px/1.5 var(--sans)}
.dark .clm{--bg:#0c0f16;--surface:#141922;--surface-2:#1b2130;--inset:#10141d;--ink:#e8ebf3;--ink-2:#9aa3b8;--ink-3:#6b7488;--border:#242b39;--border-strong:#333c4e;--accent:#8290e6;--accent-ink:#0c0f16;--accent-soft:#1f2740;--good:#5cbf90;--good-soft:#15271f;--warn:#d3a24e;--warn-soft:#2a2213;--crit:#e2705c;--crit-soft:#2c1a17;--ext:#ab90d2;--ext-soft:#221b31;--ai:#a99cf0;--ai-soft:#1c1a33;--shadow:0 1px 2px rgba(0,0,0,.4),0 12px 30px rgba(0,0,0,.4);--pop:0 14px 34px rgba(0,0,0,.55)}
...
.clm .btn{display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:6px 11px;border-radius:8px;border:1px solid var(--border-strong);background:var(--surface);color:var(--ink);font-weight:600;font-size:12px}
.clm .btn:hover{background:var(--surface-2)} .clm .btn.pri{background:var(--accent);border-color:var(--accent);color:var(--accent-ink)} .clm .btn.sm{padding:5px 9px;font-size:11.5px}
