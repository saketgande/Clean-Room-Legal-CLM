  ...
  return (
    <div className="cochat">
      ...
      <div className="cocomposer">
        <input value={input} placeholder="Describe what you need…" onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") send(); }} />
        <button className="cosend" onClick={send} disabled={!input.trim() || busy}>{ph2svg('<path d="M22 2 11 13M22 2l-7 20-4-9-9-4z"/>')}</button>
      </div>
    </div>
  );
}
