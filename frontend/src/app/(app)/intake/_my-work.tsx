...
export function MyWorkBoard({ onOpen }: { onOpen: (id: string) => void }) {
  const { data: mw } = useQuery({ queryKey: ["intake-mywork"], queryFn: intakeApi.myWork, ...POLL });
  const { data: mineData } = useQuery({ queryKey: ["intake-mine"], queryFn: intakeApi.mine, ...POLL });
