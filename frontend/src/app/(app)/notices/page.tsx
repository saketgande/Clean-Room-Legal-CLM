...
export default function NoticesPage() {
  ...
  return (
    <div className="notc">
      ...
      <div className="hd">
        <div>
          <p className="eyebrow">Operations · Legal</p>
...
const NOTC_CSS = `
.notc{--surface:#fff;--surface-2:#eef1f6;--inset:#f8fafc;--ink:#18213a;--ink-2:#586178;--ink-3:#8a92a6;--border:#e4e8f0;--border-strong:#ccd3e0;--accent:#3b4aa0;--accent-ink:#fff;--accent-soft:#eaecf8;--good:#2f875f;--good-soft:#e4f1ea;--warn:#a3690a;--warn-soft:#faf1de;--bad:#bb4835;--bad-soft:#fbeae6;--shadow:0 1px 2px rgba(20,26,40,.05),0 8px 22px rgba(20,26,40,.06);--sans:system-ui,-apple-system,"Segoe UI",Roboto,sans-serif;--mono:ui-monospace,SFMono-Regular,Menlo,monospace;padding:22px 24px 40px;color:var(--ink);font:400 13px/1.5 var(--sans)}
.dark .notc{--surface:#141922;--surface-2:#1b2130;--inset:#10141d;--ink:#e8ebf3;--ink-2:#9aa3b8;--ink-3:#6b7488;--border:#242b39;--border-strong:#333c4e;--accent:#8290e6;--accent-ink:#0c0f16;--accent-soft:#1f2740;--good:#5cbf90;--good-soft:#15271f;--warn:#e0ab54;--warn-soft:#2c2313;--bad:#e2705c;--bad-soft:#301c18;--shadow:0 1px 2px rgba(0,0,0,.4),0 10px 26px rgba(0,0,0,.4)}
...
.notc .hd .eyebrow{margin:0;font:600 10.5px var(--sans);letter-spacing:.06em;text-transform:uppercase;color:var(--ink-3)}
...
.notc .btn{display:inline-flex;align-items:center;gap:6px;padding:8px 14px;border-radius:9px;border:1px solid var(--border-strong);background:var(--surface);color:var(--ink);font-weight:600;font-size:12.5px;cursor:pointer;font-family:var(--sans)}
.notc .btn:hover{background:var(--surface-2)}
.notc .btn.pri{background:var(--accent);border-color:var(--accent);color:var(--accent-ink)}
.notc .btn.pri:hover{filter:brightness(1.06)}
.notc .btn[disabled]{opacity:.6;pointer-events:none}
...
.notc .tile.red{--tone:var(--bad);--tone-soft:var(--bad-soft)}
...
.notc .tag.red{background:var(--bad-soft);color:var(--bad)}
.notc .tag.violet{background:var(--accent-soft);color:var(--accent)}
.notc .empty{padding:40px 20px;text-align:center;font-size:13px;color:var(--ink-2)}
`;
